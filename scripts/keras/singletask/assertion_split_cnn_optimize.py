...
from assertion_split_cnn_train import get_split_cnn, split_entity_data
...
def get_random_config():
    ...
    config['num_filters'] = (random.choice(filters), random.choice(filters), random.choice(filters))
...
def run_one_eval(epochs, config, train_x, train_y, valid_x, valid_y, vocab_size, num_outputs):
    ...
    dimensions = (train_x[0].shape, train_x[1].shape, train_x[2].shape)
    model = get_split_cnn(dimensions, vocab_size, config['embed_dim'], conv_layers=config['num_filters'], fc_layers=config['layers'], filter_widths=config['filters'], num_outputs=num_outputs )
